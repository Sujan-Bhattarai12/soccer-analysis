...
# count the occurence of good, average, and bad players
player_attr %>% count(player_class)
...
tern_plot <- 
  ggtern(clean_player_data, aes(x = finishing, y = dribbling, z = strength, color = player_class)) +
  geom_point(alpha = 0.2) +
  theme_rgbw() +
  scale_color_manual(values = colors) +
  # Highlight all points with advanced players
  geom_point(data = clean_player_data %>% filter(player_class == "Advanced"), 
              aes(x = finishing, y = dribbling, z = agility)) +
  # Change legend title
  labs(color = "Players proficiency") +
  # Add labels for popular players of presnt data
  geom_text(data = clean_player_data %>% 
  ...
  geom_point(data = clean_player_data %>% 
            filter(player_name %in% c("Lionel Messi", "Cristiano Ronaldo", "Virgil van Dijk")), 
...
#save the ternary plot as image and attach it to mainframe
ggsave(plot = tern_plot, filename = "tern_plot.jpg", height = 6, width = 6)
...
ggsave(plot = g_final, filename = "ufo/infographics_draft.png", height = 16, width = 10)
```
